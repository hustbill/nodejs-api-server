// POST /v2/admin/autoships/:autoshipId
...
function getPostData(request) {
    ...
    if (body['next-autoship-date']) {
        data.nextAutoshipDate = utils.parseDate(body['next-autoship-date']);
    }
...
function post(request, response, next) {
    var context = request.context,
        updateAutoshipOptions = getPostData(request);

    async.waterfall([
        function (callback) {
            var autoshipDao = daos.createDao('Autoship', context);
            updateAutoshipOptions.autoshipId = parseInt(request.params.autoshipId, 10);
            autoshipDao.updateAutoship(updateAutoshipOptions, callback);
        }
